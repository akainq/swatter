...
- **AI-агент (ADR-0016):** анализ issue на **z.ai GLM** (суть, вероятная причина, `severity`, предложение по фиксу) — **только по запросу** (кнопка «Проанализировать» в деталке / POST-эндпоинт; авто-запуск отключён 2026-07-03 — контроль расхода токенов); результат в деталке issue и, если уже готов, в тексте Telegram-алерта. Фича опциональна: нет `ZAI_API_KEY` → выключена.

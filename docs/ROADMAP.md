...
## M4 — Tracing / performance (закрывает MVP) — в работе

Срезы: (1) ADR-0014 + таблица spans + разбор `transaction` в пайплайне + conformance реальным SDK; (2) список транзакций (p50/p95/rpm) API+UI; (3) деталка транзакции + trace waterfall; (4) error↔trace («Related errors», ошибки в waterfall, переходы).
...
- Кросс-сервисная связка ошибок по `trace_id` (фронт ↔ бэк ↔ микросервисы, «Связанные ошибки» в деталке): SDK уже пропагируют trace-контекст (`sentry-trace`/`baggage`), `trace_id` уже хранится в CH — покрывается этой вехой, отдельный ADR по correlation-ID не нужен (решение 2026-07-03).

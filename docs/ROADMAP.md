...
Канал — **только Telegram** (решение 2026-07-02): общий бот на инстанс (`TELEGRAM_BOT_TOKEN`), per-project `chat_id`; email/Slack/webhook в MVP не делаем. AI — **только по запросу** (поправка 2026-07-03 в ADR-0016: контроль расхода токенов).

- [x] **Срез 1 (2026-07-02):** Oban поверх PG (очереди `alerts`/`ai`, в тестах `:manual`; предусмотрен ADR-0002), runtime-конфиг секретов (`TELEGRAM_BOT_TOKEN`, `ZAI_API_KEY`, `ZAI_MODEL`=glm-4.6, `ZAI_BASE_URL`, cooldown), таблица `project_alert_settings` + контекст `Swatter.Alerts`; fail-safe — без токена/`chat_id` алерты проекта выключены.
- [x] **Срез 2 (2026-07-02):** Telegram-доставка — `upsert_from_event` помечает issue виртуальным `event_kind` (new|regression|ongoing), пайплайн ставит Oban-джобу `NotifyWorker` → `sendMessage` через Req (4xx → cancel, сеть/5xx → ретрай); cooldown per-(issue, rule) в Redis (`SET NX EX`, fail-open как ADR-0009); текст с deep-link на issue. Проверено живой отправкой реальному боту.
- [x] **Срез 3 (2026-07-02):** порог частоты — fixed-window `INCR` в Redis, алерт ровно на пороговом событии; ETS-кэш настроек (`SettingsCache`, TTL 30 с, инвалидация при записи) — частотное правило смотрит каждое событие без PG на горячем пути.
- [x] **Хост события (2026-07-03):** top-level `server_name` продвигается в теги (как в Sentry; явный SDK-тег приоритетнее) → виден в UI и попадает строкой `host:` в алерт.
- [x] **Срез 4 (2026-07-03):** AI-анализ на z.ai GLM (ADR-0016) — `Swatter.AI.request_analysis` (pending-строка + Oban-джоба с уникальностью 60 с) → промпт из issue + последнего события CH (символикованный in-app стек, breadcrumbs, теги) → строгий JSON `summary`/`probable_cause`/`severity`/`suggested_fix` в `issue_ai_analyses`; готовый анализ подмешивается в Telegram-текст (алерт никогда не ждёт LLM). Живой прогон против настоящего glm-4.6 через реальную Oban-очередь.
- [x] **Срез 5 (2026-07-03):** API + UI — `GET/PUT /api/0/projects/{org}/{proj}/alert-settings`, `POST /api/0/issues/{id}/analyze` (202/422), `aiAnalysis`+`aiEnabled` в деталке issue; страница Alerts проекта (chat_id, правила, порог; предупреждение, если токен не задан) и AI-панель в деталке (Analyze/Re-analyze, severity-бейдж, причина/фикс). Проверено живым браузером end-to-end: chat_id сохранён из UI → реальное событие через работающий ingest → настоящий Telegram-алерт с `host:`; AI-панель отрисовала сохранённый разбор glm-4.6.
- Ingest `session(s)` items → crash-free rate — перенесено в пост-MVP (не блокирует алерты).

**Готово, когда:** команда узнаёт о новой ошибке из Telegram (с AI-резюме) раньше, чем из UI; в деталке issue виден AI-разбор. → **M3 закрыт 2026-07-03.**

## M4 — Tracing / performance (закрывает MVP)

- [x] **Срез 1 (2026-07-03):** ADR-0014 — одна CH-таблица `spans` (транзакция = корневой span `is_segment=1`, `transaction_name` денормализовано, TTL 30 дней, bloom-index по `trace_id`); `SpanBuilder` разбирает `transaction`-items в пайплайне (валидация hex-id, «ядовитое» дропается), батчер делает две `insert_all` из одного батча; транзакции не создают issues и не алертят. Conformance: реальный `@sentry/node` с `tracesSampleRate=1` довёз транзакцию до спанов (корень + дочерние, тайминги сходятся).
- [x] **Срез 2 (2026-07-03):** список транзакций — `Spans.transaction_stats` (quantile 0.5/0.95 по корневым строкам на лету, окно 1h/24h/7d, rpm), `GET .../performance/transactions`, страница Performance (таблица rpm/p50/p95/count, переключатель окна). Проверено живым браузером на реальных транзакциях SDK.
- [x] **Срез 3 (2026-07-03):** деталка транзакции (последние трейсы, медленные сверху) + **trace waterfall**: `GET /organizations/{org}/traces/{id}` — спаны по всем проектам организации (кросс-сервисно, слаги из PG для бейджей), дерево по `parent_span_id`, бары относительно окна трейса. Живой waterfall сошёлся с реальными sleep'ами скрипта.
- [x] **Срез 4 (2026-07-03):** error↔trace — bloom-index по `trace_id` в `events`, `Events.related_by_trace` (ошибки трейса поперёк проектов организации), ошибки в ответе трейса + `GET .../traces/{id}/errors`; в waterfall — красный бейдж «N err» на сегменте сервиса и список ошибок с переходами в issues; в деталке issue — секция Trace («View trace» + related errors других сервисов, кросс-проектно). Проверено живым браузером в обе стороны.
- Сэмплинг — на стороне SDK (`traces_sample_rate`); динамическое серверное сэмплирование — пост-MVP.

**Готово, когда:** по медленному запросу виден waterfall спанов и связанные ошибки. → **M4 закрыт 2026-07-03. MVP завершён: ошибки+issues (M1), sourcemaps+releases (M2), Telegram-алерты+AI (M3), tracing (M4).**
